...
void HariMain(void)
{
	wanna_write = (int *) WANNA_WRITE_ADDR;
	...
	struct TASK *task_a, *task, *yuyama_task;
	...
	*((int *) 0x0fec) = (int) &fifo;
	...
	memman_init(memman);
	memman_free(memman, 0x00001000, 0x0009e000); /* 0x00001000 から 0x0009efff まで */
	...
	*((int *) 0x0fe4) = (int) shtctl;
	...
	for (;;) {
		yuyama_times++;
		if (yuyama_task != 0 && *wanna_write == 1) {
			sprintf(s, "%d and %d\n", fifo32_status(&yuyama_task->fifo), yuyama_times);
			cons_putstr0(yuyama_task->cons, s);
			*wanna_write = 2;
		}
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (new_mx >= 0) {
				...
				new_wx = 0x7fffffff;
						...
						if (yuyama_task != 0 && *wanna_write == 4) {
            				sprintf(s, "%d and %d\n", fifo32_status(&yuyama_task->fifo), yuyama_times);
            				cons_putstr0(yuyama_task->cons, s);
            				*wanna_write = 5;
            			}
				task_sleep(task_a, yuyama_task, wanna_write, yuyama_times); // こいつが犯人！！！！！！！！！！
				if ( *wanna_write == 5) {
				*wanna_write = 100;
				}
			...
			if (yuyama_task != 0 && *wanna_write == 2) {
				sprintf(s, "%d and %d\n", fifo32_status(&yuyama_task->fifo), yuyama_times);
				cons_putstr0(yuyama_task->cons, s);
				*wanna_write = 3;
			}
									...
									if (sht->buf[y * sht->bxsize + x] != sht->col_inv) {
										...
										if (sht->bxsize - 21 <= x && x < sht->bxsize - 5 && 5 <= y && y < 19) {
											...
											if ((sht->flags & 0x10) != 0) {		/* アプリが作ったウィンドウか？ */
												...
												yuyama_task = sht->task;
												task = sht->task;
												io_cli();
												// なんかfifoがcolorのシートと同じ？
												fifo32_put(&task->fifo, 4);
												fifo32_put(&task->fifo, 5);
												fifo32_put(&task->fifo, 6);
												sprintf(s, "%d and %d\n", fifo32_status(&task->fifo), yuyama_times);
												cons_putstr0(task->cons, s);
//												fifo32_get(&task->fifo);
//												char a = fifo32_get(&task->fifo);
//												sprintf(s, "%d", task->fifo);
//												cons_putstr0(task->cons, s);
												io_sti();
												*wanna_write = 1;
											}
				...
				cons_putstr0((shtctl->sheets0 + (i - 768))->task->cons, "\nyuyama :\n");
				close_console(shtctl->sheets0 + (i - 768));
			}
		}

									if (yuyama_task != 0 && *wanna_write == 3) {
                            			*wanna_write = 4;
                            		}
	}
}
